...
void t_hs_generator::generate_service_client(t_service* tservice) {
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    string fname = (*f_iter)->get_name();
    indent(f_client_) << "T.writeMessage op (\"" << fname << "\", T.M_CALL, seqn)" << endl;
    ...
    if (!(*f_iter)->is_oneway()) {
      ...
      indent(f_client_) << "(fname, mtype, rseqid) <- T.readMessage ip" << endl;
...
void t_hs_generator::generate_service_server(t_service* tservice) {
  ...
  indent(f_service_) << "(name, typ, seqid) <- T.readMessage iprot" << endl;
...
void t_hs_generator::generate_process_function(t_service* tservice,
  ...
  if (tfunction->is_oneway()) {
    ...
    indent(f_service_ ) << "T.writeMessage oprot (\"" << tfunction->get_name() << "\", T.M_REPLY, seqid)" << endl;
  ...
  if (n > 0) {
    ...
    if (!tfunction->is_oneway()) {
      indent(f_service_) << "T.writeMessage oprot (\"" << tfunction->get_name() << "\", T.M_EXCEPTION, seqid)" << endl;
